...
class Random2DGaussian:
    ...
    def __init__(self):
        dwx,dwy = self.maxx-self.minx, self.maxy-self.miny
        self.means = (self.minx,self.miny)
        self.means += np.random.random_sample(2)*(dwx,dwy)
        eigvals = np.random.random_sample(2)
        eigvals *= (dwx/self.scalecov, dwy/self.scalecov)
        eigvals **= 2
        theta = np.random.random_sample()*np.pi*2
        R = [[np.cos(theta), -np.sin(theta)], 
         [np.sin(theta), np.cos(theta)]]
        self.sigma = np.dot(np.dot(np.transpose(R), np.diag(eigvals)), R)

    def get_sample(self, n):
        return np.random.multivariate_normal(self.means, self.sigma, n)
